...
Train_patient_index = os.listdir("16k/train_speech")
Dev_patient_index = os.listdir("16k/dev_speech")
Test_patient_index = os.listdir("16k/test_speech")

supervised_data = pd.read_excel("all_PVQD_mode.xlsx")
...
for i in tqdm(Train_patient_index):
	...
	for row in supervised_data.iterrows():
		if patient_index == row[1]["File"].strip():
			...
			with open("label/train_speech_reg.csv","a+") as csvfile:
				writer =csv.writer(csvfile)
				input_data = ["train_speech/"+i,row[1]["Average_G"],row[1]["Average_R"],row[1]["Average_B"],row[1]["Average_A"],row[1]["Average_S"]]
...
for i in tqdm(Dev_patient_index):
	...
	for row in supervised_data.iterrows():
		if patient_index == row[1]["File"].strip():
			# print(row[1]["File"])
			# print(row[1]["File"]==patient_index)
			with open("label/dev_speech_reg.csv","a+") as csvfile:
				writer =csv.writer(csvfile)
				input_data = ["dev_speech/"+i,row[1]["Average_G"],row[1]["Average_R"],row[1]["Average_B"],row[1]["Average_A"],row[1]["Average_S"]]
				# print(input_data)
				writer.writerow(input_data)
...
for i in tqdm(Test_patient_index):
	...
	for row in supervised_data.iterrows():
		if patient_index == row[1]["File"].strip():
			# print(row[1]["File"])
			# print(row[1]["File"]==patient_index)
			with open("label/test_speech_reg.csv","a+") as csvfile:
				writer =csv.writer(csvfile)
				input_data = ["test_speech/"+i,row[1]["Average_G"],row[1]["Average_R"],row[1]["Average_B"],row[1]["Average_A"],row[1]["Average_S"]]
				# print(input_data)
				writer.writerow(input_data)
